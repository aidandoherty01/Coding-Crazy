...
export async function callGetPlayerData(id) {
  try {
    const response = await fetch(
      `https://coding-crazy.onrender.com/get-player-data/${id}`
    );
...
export async function callAddPlayer(playerData) {
  try {
    const response = await fetch(
      "https://coding-crazy.onrender.com/add-player",
      {
        method: "POST",
...
export async function callRemovePlayer(id) {
  try {
    const response = await fetch(
      `https://coding-crazy.onrender.com/remove-player/${id}`,
      {
        method: "DELETE",
      }
    );
...
export async function callUpdatePlayerInfo(id, newLoc, x, y, numAPlusses) {
  try {
    const response = await fetch(
      "https://coding-crazy.onrender.com/update-player-info",
      {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
