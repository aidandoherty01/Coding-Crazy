...
*/
export async function exportCollectionToJson(collectionName = _studySetCollection, filePath = _exportPath) {  // async requires the function to return a 'promise' [https://www.w3schools.com/js/js_promise.asp]
...
export async function exportUniqueSubjectsToJson(filePath = _exportPath) {
  ...
  try {
    ...
    console.log(`Subjects from '${_studySetCollection}' exported to '${filePath}'`);
  } catch (err) {
    console.error('Error importing data: ', err);
  } finally {
    await client.close();
...
export async function exportStudySetToJson(_subject, filePath = _questionsPath) {
  const client = new MongoClient(uri, { monitorCommands : true });  // Initialize MongoClient class (with debugging enabled)
  ...
  try {
    ...
    console.log(`${_subject} questions from '${_studySetCollection}' exported to '${filePath}'`);
  } catch (err) {
    console.error('Error importing data: ', err);
  } finally {
    await client.close();
...
export async function exportAccountToJson(_username, _password, filePath = _exportPath) {
  const client = new MongoClient(uri, { monitorCommands : true });  // Initialize MongoClient class (with debugging enabled)
  ...
  try {
    ...
    const user = await collection.findOne({ username: _username, password: _password });
    if (!user) { throw new Error(`Incorrect combo bruh`); }
